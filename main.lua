...
道具生成改动（以下道具不会野生生成，重Roll或者其他途径生成不受影响）：
- 恶魔房
    - 我们需要深入挖掘！
- 商店
    - 残损铁镐
...
道具效果改动：
- 皇帝卡：效果改为由当前房间向BOSS方向前进4格，持有[塔罗牌桌布]时改为6格，虚空层不受影响
- 四面骰：生效时，视为拥有[混沌]
- 诅咒硬币：触发时掉落
...
- 堕化遗骸偶尔被扔到高柱子里出不来的问题
- 堕化雅各快速双击副手主动时无事发生的问题

其他改动：
- 获得失忆症药丸时，会立即将其识别
- 黑暗诅咒和迷途诅咒不会自然生成，使用药丸获得的不受影响
- 对游戏中大量烦人的动画进行了加速处理
- 当献祭次数达到11次后，献祭房的刺会缩回
...
- 房间雷达功能，按住地图键时生效，在特定情况下会显示：
    ...
    - 房间内有多组多选一道具时，会显示所有的道具底座，并给同组道具赋予相同的颜色滤镜
- 快速启用、禁用控制台功能：按下~键启用控制台，按下ctrl+~禁用控制台
- 开启鼠标操控时，鼠标右键会映射为丢弃键（Ctrl键）
- 持有[捕梦网]时，将会在靠近下层入口时进行醒目提示
...
v1.10.0：
新增：
- 在信息面板中增加一个二维码，方便机器人识别当前状态

修复：
- [四面骰]生效时，能听见或看见魂火破灭的效果的问题
- [编号丢失]生效时，会带有[混沌]的效果的问题

其他：
- 结算后，清除并免疫所有诅咒
- 结算完成后，允许直接使用超级种子而无需新开局
----------------------------------------------------------------
v1.9.0：
新增：
- 持有[捕梦网]时，会在靠近下层入口时进行醒目提示的功能
- 修复了游戏bug：堕化雅各快速双击副手主动时无事发生的问题
- 锁定当前角色，重新随机一个种子的功能

修复：
- 在祸兽路线不小心通过[十二面骰]+反人类卡+[妈咪炸弹！]+[牌意解读]进入回溯版本的死寂层时，mod会报错的问题
- [教条]的减伤效果对究极贪婪的大金币弹幕不起作用的问题
- 某些情况下拾取[伊甸的祝福]，会在下一局生效并给予道具的问题

其他：
- 将究极贪婪战斗的房间风格设置为和贪婪模式相同
- 角色为雅各&以扫时，将左下角信息区域下移2行，避免挡住恶魔房/天使房概率
- 准备状态下选择终点并开始游戏时，会自动重新开局，防止[红豆汤]等与时间相关的道具受到影响的问题
- 因死亡而导致游戏结束时，自动暂停计时器
- 屏幕震动时，文本会跟随震动
- 所有按钮的文字都调整为居中显示
- 准备状态下，让角色无敌，防止极端情况下受伤或死亡
- 优化了替换背景图片界面的提示文本
----------------------------------------------------------------
v1.8.0：
新增：
- 祸兽路线结算后，传送至第一层，方便查看属性和道具列表
- 如果房间内有多组多选一道具，房间雷达会显示所有的道具底座，并给同组道具赋予相同的颜色滤镜
- 为究极贪婪终点添加了[毛毯]的兼容

调整：
- 【重要】不再降低任何道具的权重；之前降低权重的道具，统一改为无法野生生成，重Roll或其他途径生成不受影响
- 现在[编号丢失]也无法阻止[四面骰]视为拥有[混沌]的效果了

修复：
- 母亲路线自动开启支线门时，如果有蓝钥匙饰品，会进入蓝房间的问题
- 究极贪婪房间可以使用[七面骰]导致不会生成奖杯的问题
- 通过[发光沙漏]进行某些操作可以使得皇帝卡传送8格的问题
- 在只连接特殊房间的红房间或者究极隐藏中使用皇帝卡无法正确触发的问题
- 在死亡证明或者矿层逃亡区域中使用皇帝卡无法正确触发的问题

其他：
- 如果背景图片的尺寸大于480x320，则强行裁剪显示，防止显示不正常
----------------------------------------------------------------
v1.7.0：
新增：
- 当开启滤镜时，在柱状图和信息面板使用大字体，防止文字模糊
- 开启鼠标操控时，鼠标右键会映射为丢弃键（Ctrl键）
- 当献祭次数达到11次后，献祭房的刺会缩回

其他：
- 当开启滤镜时，使用大字体显示关闭滤镜的提示
- 将主窗口的尺寸改回之前版本，以适配背景图片的尺寸，并微调布局
- 精神错乱终点禁止进入教堂/阴间
- 微调了信息面板的布局和显示效果
----------------------------------------------------------------
v1.6.1：
...
v1.6.0：
新增：
- 【重要】母亲路线下，如果缺少菜刀碎片，不会再禁止下层了，而是改为不自动开启支线门
- 母亲路线下，如果在没有集齐菜刀碎片的情况下击败妈妈，允许进入子宫II
- 修复了游戏bug：堕化遗骸有时会被扔到高柱子里面捡不到的问题
- 查看插件列表的功能
- 练习模式：开启后不需要鼠标就能暂停计时器，在正赛中使用会判定为犯规
...
- 母亲路线下，矿层只需要踩下一个按钮即可进入逃亡区域
- 母亲路线下，白火、镜子、刀柄、矿车房间的位置会在地图上点亮
